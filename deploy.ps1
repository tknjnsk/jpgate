...
# --- 相場 -------------------------------------------------------------------
# eBay の相場を少しずつ引き直す。**publish より前**に置くこと（ページに
# 載せる数字なので、後ろだと1周遅れになる）。
# 認証が無ければ中で諦めて 0 を返すので、ここでは分岐しない。
python -m jpgate prices
if ($LASTEXITCODE -ne 0) { Write-Warning "相場の取得に失敗しました。" }
